...
public class SoundCullingConfigScreen extends Screen {
    ...
    private static String actionLabel(String action) {
        String normalized = SoundCullingConfig.normalizeAction(action);

        return tr("soundculling.ui.action." + normalized.toLowerCase(Locale.ROOT));
    }
    ...
    private List<SoundCullingTracker.RecentSoundSnapshot> recentSounds = List.of();
    private int liveRefreshTicks;
    ...
    public SoundCullingConfigScreen(Screen parent) {
        ...
        this.parent = parent;
        this.config = SoundCulling.getConfig().copy();
    ...
    public void tick() {
        super.tick();
        if (page != Page.LIVE || ++liveRefreshTicks < 10) {
            return;
        }
        liveRefreshTicks = 0;
        List<SoundCullingTracker.RecentSoundSnapshot> latest =
                SoundCullingTracker.getRecentSounds(32);
        recentSounds.replaceAll(
                previous -> {
                    for (SoundCullingTracker.RecentSoundSnapshot current : latest) {
                        if (current.soundId().equals(previous.soundId())) {
                            return current;
                        }
                    }
                    return previous;
    ...
    private void refreshUI() {
        ...
        addRenderableWidget(
                Button.builder(
                                Component.literal(tr("soundculling.ui.button.save_close")),
                                button -> {
                                    applyAndSave();
                                    onClose();
                                })
                        .bounds(right - saveWidth, bottomY, saveWidth, 18)
                        .build());

        addRenderableWidget(
                Button.builder(
                                Component.literal(tr("soundculling.ui.button.cancel")),
                                button -> onClose())
                        .bounds(right - saveWidth - 6 - cancelWidth, bottomY, cancelWidth, 18)
                        .build());
    ...
    private void buildOverview(Layout layout) {
        ...
        addRenderableWidget(
                Button.builder(
                                Component.literal(
                                        enabled
                                                ? tr("soundculling.ui.sound_culling.on")
                                                : tr("soundculling.ui.sound_culling.off")),
                                button -> {
                                    enabled = !enabled;
                                    refreshUI();
                                })
        ...
        addRenderableWidget(
                Button.builder(
                        ...
                        .tooltip(
                                Tooltip.create(
                                        Component.literal(
                                                tr("soundculling.ui.tooltip.preset_cycle"))))
                        .build());
    ...
    private void buildPerformance(Layout layout) {
        ...
        addNumRow(
                ...
                tr("soundculling.ui.label.adaptive_start"),
                tr("soundculling.ui.tooltip.adaptive_start"));
        ...
        addNumRow(
                ...
                tr("soundculling.ui.label.adaptive_full"),
                tr("soundculling.ui.tooltip.adaptive_full"));
        ...
        addNumRow(
                ...
                tr("soundculling.ui.label.default"),
                tr("soundculling.ui.tooltip.default"));
        ...
        addNumRow(
                ...
                tr("soundculling.ui.label.blocks"),
                tr("soundculling.ui.tooltip.blocks"));
        ...
        addNumRow(
                ...
                tr("soundculling.ui.label.hostile"),
                tr("soundculling.ui.tooltip.hostile"));
        ...
        addNumRow(
                ...
                tr("soundculling.ui.label.neutral"),
                tr("soundculling.ui.tooltip.neutral"));
        ...
        addNumRow(
                ...
                tr("soundculling.ui.label.ambient"),
                tr("soundculling.ui.tooltip.ambient"));
    ...
    private void buildLive(Layout layout) {
        ...
        int listWidth = Math.max(150, leftCardRight - left - innerPad);

        int maxRecent = layout.panelHeight >= 270 ? 6 : 5;

        recentSounds = SoundCullingTracker.getRecentSounds(maxRecent);

        int y = top;

        for (SoundCullingTracker.RecentSoundSnapshot sound : recentSounds) {
            String label =
                    trim(sound.soundId(), 24) + "  x" + sound.triggered() + "  c" + sound.culled();
            ...
            addRenderableWidget(
                    Button.builder(
                            ...
                            .tooltip(Tooltip.create(Component.literal(sound.soundId())))
                            .build());

            y += 21;
        }

        boolean hasSelection = selectedSoundId != null && !selectedSoundId.isBlank();
        ...
        addRenderableWidget(
                Button.builder(
                                ...
                                button -> refreshUI())
                        .bounds(actionX, top + 70, actionAreaWidth, 18)
                        .build());
    }

    private Button actionButton(
            int x, int y, int width, String label, boolean active, Runnable action) {
        Button button =
                Button.builder(
                                Component.literal(label),
                                ignored -> {
    ...
    private void buildAdvanced(Layout layout) {
        ...
        addNumRow(
                ...
                tr("soundculling.ui.tooltip.region_total"));
        ...
        addNumRow(
                ...
                tr("soundculling.ui.tooltip.window_ticks"));
        ...
        addNumRow(
                ...
                tr("soundculling.ui.tooltip.region_size"));
        ...
        addNumRow(
                ...
                tr("soundculling.ui.tooltip.neighbour_cells"));
                ...
                Button.builder(
                                Component.literal(
                                        label
                                                + ": "
                                                + (value
                                                        ? tr("soundculling.ui.state.on")
                                                        : tr("soundculling.ui.state.off"))),
                                button -> {
                                    setter.set(!value);
                                    markCustom();
                                    refreshUI();
    ...
    private void addRule(String pattern, String action) {
        ...
        rules.add(
                new SoundCullingConfig.SoundRule(
                        pattern, SoundCullingConfig.normalizeAction(action)));
        preset = "CUSTOM";
    ...
    private void removeRule(String pattern) {
        ...
        boolean removedExact =
                rules.removeIf(rule -> rule != null && pattern.equalsIgnoreCase(rule.pattern));

        String removedPattern = removedExact ? pattern : null;

        if (!removedExact) {
            String namespacePattern = SoundRuleEngine.namespacePattern(pattern);

            if (namespacePattern != null
                    && !namespacePattern.isBlank()
                    && !namespacePattern.equalsIgnoreCase(pattern)) {
                boolean removedNamespace =
                        rules.removeIf(
                                rule ->
                                        rule != null
                                                && namespacePattern.equalsIgnoreCase(rule.pattern));
    ...
    private void cyclePreset() {
        String next =
                switch (preset == null ? "BALANCED" : preset) {
    ...
    private String prettyPreset() {
        String normalized =
                preset == null || preset.isBlank() ? "BALANCED" : preset.toUpperCase(Locale.ROOT);

        return switch (normalized) {
            case "PERFORMANCE" -> tr("soundculling.ui.preset.performance");

            case "AGGRESSIVE" -> tr("soundculling.ui.preset.aggressive");

            case "CUSTOM" -> tr("soundculling.ui.preset.custom");

            default -> tr("soundculling.ui.preset.balanced");
        };
    }

    @Override
    public void extractRenderState(
            GuiGraphicsExtractor extractor, int mouseX, int mouseY, float delta) {
        ...
        extractor.text(
                font, Component.literal(title), layout.panelX + 14, layout.panelY + 11, 0xFFF4F7FB);

        if (layout.panelWidth >= 420) {
            extractor.text(
                    font,
                    Component.literal(subtitle),
                    layout.panelX + layout.panelWidth - font.width(subtitle) - 14,
                    layout.panelY + 11,
                    0xFF8290A0);
        }
        ...
        int activeX = tabX + selectedIndex * (tabWidth + tabGap);

        extractor.fill(activeX + 5, tabY + 19, activeX + tabWidth - 5, tabY + 20, 0xFF55A7FF);
    ...
    private void renderModernControls(
            GuiGraphicsExtractor extractor, int mouseX, int mouseY, Layout layout) {
        int tabGap = 4;
        int tabX = layout.panelX + 14;
        int tabY = layout.panelY + 36;
        int tabWidth = (layout.panelWidth - 28 - tabGap * 3) / 4;
        ...
        int rightEdge = layout.panelX + layout.panelWidth - 14;

        int saveX = rightEdge - saveWidth;

        int cancelX = saveX - 6 - cancelWidth;
        ...
        switch (page) {
            ...
            case PERFORMANCE -> {
                ...
                drawNumberControls(extractor, left, y, mouseX, mouseY);

                drawNumberControls(extractor, left, y + row, mouseX, mouseY);

                drawNumberControls(extractor, left, y + row * 2, mouseX, mouseY);

                drawNumberControls(extractor, left, y + row * 3, mouseX, mouseY);

                drawNumberControls(extractor, right, y, mouseX, mouseY);

                drawNumberControls(extractor, right, y + row, mouseX, mouseY);

                drawNumberControls(extractor, right, y + row * 2, mouseX, mouseY);
            ...
            case LIVE -> {
                ...
                for (SoundCullingTracker.RecentSoundSnapshot sound : recentSounds) {
                    boolean selected = sound.soundId().equals(selectedSoundId);

                    drawLiveSoundRow(
                            extractor, left, listY, listWidth, 18, sound, selected, mouseX, mouseY);
    ...
    private void drawNumberControls(
            GuiGraphicsExtractor extractor, int x, int y, int mouseX, int mouseY) {
        drawModernButton(extractor, x + 92, y, 18, 18, "-", false, true, mouseX, mouseY);

        drawModernButton(extractor, x + 152, y, 18, 18, "+", false, true, mouseX, mouseY);
    }
    ...
    private void drawToggleSkin(
        ...
        int border = state ? 0xFF55A7FF : 0xFF374352;

        int background = hover ? 0xFF283545 : 0xFF202833;

        extractor.fill(x, y, x + width, y + height, border);

        extractor.fill(x + 1, y + 1, x + width - 1, y + height - 1, background);

        extractor.text(font, Component.literal(label), x + 8, y + 5, 0xFFE6EBF1);

        String stateText = state ? tr("soundculling.ui.state.on") : tr("soundculling.ui.state.off");

        int pillWidth = 29;
        int pillHeight = 12;

        int pillX = x + width - pillWidth - 8;

        int pillY = y + (height - pillHeight) / 2;

        int pillColor = state ? 0xFF24679B : 0xFF343E49;

        extractor.fill(pillX, pillY, pillX + pillWidth, pillY + pillHeight, pillColor);

        extractor.text(
                font,
                Component.literal(stateText),
                pillX + (pillWidth - font.width(stateText)) / 2,
                pillY + 2,
                state ? 0xFFFFFFFF : 0xFFADB5BE);
    }
    ...
    private void drawSelectorSkin(
        ...
        extractor.fill(x, y, x + width, y + height, 0xFF374352);

        extractor.fill(
                x + 1, y + 1, x + width - 1, y + height - 1, hover ? 0xFF283545 : 0xFF202833);

        extractor.text(font, Component.literal(label), x + 8, y + 5, 0xFF9DA8B4);

        extractor.text(
                font,
                Component.literal(value),
                x + width - font.width(value) - 8,
                y + 5,
                0xFF65B5FF);
    }
    ...
    private void drawLiveSoundRow(
            ...
            int mouseX,
            int mouseY) {
        drawModernButton(extractor, x, y, width, height, "", selected, true, mouseX, mouseY);

        String triggeredText = "x" + sound.triggered();

        String culledText = "c" + sound.culled();

        int padding = 7;
        int counterGap = 10;
        int idGap = 10;

        int culledX = x + width - padding - font.width(culledText);

        int triggeredX = culledX - counterGap - font.width(triggeredText);

        int idX = x + padding;

        int idAvailableWidth = Math.max(0, triggeredX - idGap - idX);

        String visibleId = trimToPixelWidth(sound.soundId(), idAvailableWidth);

        int textY = y + Math.max(2, (height - 8) / 2);

        int idColor = selected ? 0xFFFFFFFF : 0xFFE8EDF2;

        extractor.text(font, Component.literal(visibleId), idX, textY, idColor);
        ...
        extractor.text(
                ...
                triggeredX,
                textY,
                selected ? 0xFFFFFFFF : 0xFFB6C0CA);
        ...
        extractor.text(
                ...
                culledX,
                textY,
                sound.culled() > 0 ? 0xFFFFA0A0 : (selected ? 0xFFFFFFFF : 0xFF9DA8B4));
    }

    private String trimToPixelWidth(String text, int maxWidth) {
    ...
    private void drawModernButton(
            ...
            int mouseX,
            int mouseY) {
        boolean hover = active && isInside(mouseX, mouseY, x, y, width, height);
        ...
        if (!active) {
            ...
            border = 0xFF55A7FF;
            background = hover ? 0xFF28577E : 0xFF214866;
            textColor = 0xFFFFFFFF;
        } else {
            border = 0xFF374352;
            background = hover ? 0xFF2A3746 : 0xFF202833;
        ...
        int maxTextWidth = Math.max(8, width - 12);
        ...
        if (font.width(text) > maxTextWidth) {
            ...
            while (end > 1 && font.width(source.substring(0, end) + suffix) > maxTextWidth) {
                end--;
            }

            text = source.substring(0, Math.max(1, end)) + suffix;
        }
    ...
    private void renderOverview(GuiGraphicsExtractor extractor, Layout layout) {
        ...
        drawStatCard(
                ...
                Math.round(SoundCullingTracker.getAdaptivePressure() * 100.0f) + "%");
        ...
        drawStatCard(
                ...
                String.valueOf(SoundCullingTracker.getTotalCulled()));
        ...
        drawStatCard(
                ...
                String.valueOf(SoundCullingTracker.getTotalDampened()));
    }

    private void renderPerformance(GuiGraphicsExtractor extractor, Layout layout) {
        int left = layout.panelX + 14 + (layout.panelWidth / 2 - 188) / 2;
        int right = layout.panelX + layout.panelWidth / 2 + 4 + (layout.panelWidth / 2 - 188) / 2;
        int y = layout.panelY + 82;
        int row = 24;
        ...
        renderValue(
                ...
                y,
                tr("soundculling.ui.label.adaptive_start"),
                tr("soundculling.ui.value.per_second", adaptiveStart));
        ...
        renderValue(
                ...
                y + row,
                tr("soundculling.ui.label.adaptive_full"),
                tr("soundculling.ui.value.per_second", adaptiveFull));
        ...
        renderValue(
                ...
                y + row * 2,
                tr("soundculling.ui.label.ambient"),
                String.valueOf(limitAmbient));
    }

    private void renderLive(GuiGraphicsExtractor extractor, Layout layout) {
        int contentLeft = layout.panelX + 14;
        int contentRight = layout.panelX + layout.panelWidth - 14;
        ...
        int recentTitleX =
                contentLeft + (leftCardRight - contentLeft - font.width(recentTitle)) / 2;

        extractor.text(font, Component.literal(recentTitle), recentTitleX, headerY, 0xFFAAB4C0);
        ...
        int actionTitleX = actionCardX + (actionCardWidth - font.width(actionTitle)) / 2;

        extractor.text(
                font, Component.literal(actionTitle), actionTitleX, headerY, actionTitleColor);
        ...
        if (!ruleStatus.isBlank()) {
            extractor.text(
                    ...
                    layout.panelY + layout.panelHeight - 49,
                    0xFF7EE787);
        }
    }

    private void renderAdvanced(GuiGraphicsExtractor extractor, Layout layout) {
        int left = layout.panelX + 14 + (layout.panelWidth / 2 - 188) / 2;
        int right = layout.panelX + layout.panelWidth / 2 + 4 + (layout.panelWidth / 2 - 188) / 2;
        int y = layout.panelY + 82;
        int row = 24;
        ...
        String rulesText = tr("soundculling.ui.advanced.rules", rules.size());
    ...
    private void drawStatCard(
            ...
            String label,
            String value) {
        extractor.fill(x, y, x + width, y + height, 0xFF1D2530);

        extractor.fill(x, y, x + 2, y + height, 0xFF55A7FF);

        extractor.text(font, Component.literal(label), x + 10, y + 6, 0xFF98A5B3);
    ...
    private void renderValue(
            GuiGraphicsExtractor extractor, int x, int y, String label, String value) {
        extractor.text(font, Component.literal(label), x, y + 5, 0xFFC5CDD6);

        extractor.text(
                font, Component.literal(value), x + 131 - font.width(value) / 2, y + 5, 0xFFFFFFFF);
    }
    ...
    public void extractBackground(
        ...
        extractor.fill(0, 0, width, height, 0xD6080A0E);
        ...
        switch (page) {
            ...
            case PERFORMANCE -> {
                ...
                extractor.fill(
                        layout.panelX + 14, contentTop, middle - 4, contentBottom, 0xB8171D25);
            ...
            case LIVE -> {
                int actionWidth = 188;
                int actionX = layout.panelX + layout.panelWidth - actionWidth - 14;

                extractor.fill(
                        layout.panelX + 14, contentTop, actionX - 7, contentBottom, 0xB8171D25);
    ...
    private static String trim(String value, int max) {
        ...
        return value.substring(0, Math.max(1, max - 1)) + "â€¦";
    }
